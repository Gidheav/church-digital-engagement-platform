...
const AppearanceTab = memo<AppearanceTabProps>(({ form, supportedIcons, onFormUpdate }) => (
  <div className="flex flex-col lg:flex-row gap-5 items-start">
    <div className="flex-1 flex flex-col gap-5">
      ...
      <div className="bg-white rounded-xl border border-slate-200">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2.5">
          ...
          <h3 className="text-sm font-bold text-slate-800">Card Icon</h3>
        </div>
        <div className="px-6 py-6">
          <p className="text-xs text-slate-500 mb-4">Select the icon displayed on the giving card. This uses Material Symbols icons.</p>
          <div className="grid grid-cols-8 md:grid-cols-12 gap-2">
            {AVAILABLE_ICONS.map(iconName => (
              <button
                ...
                title={iconName}
                className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${form.icon === iconName ? 'bg-primary text-white shadow-sm ring-2 ring-primary/30' : 'bg-slate-100 text-slate-500 hover:bg-primary/10 hover:text-primary'}`}
              >
      ...
      <div className="bg-white rounded-xl border border-slate-200">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2.5">
          ...
          <h3 className="text-sm font-bold text-slate-800">Cover Image</h3>
        </div>
        <div className="px-6 py-6">
          <p className="text-xs text-slate-500 mb-4">Used on featured cards or when a background image is needed. Recommended: 1280×720px.</p>
            ...
            {form.cover_image ? (
              ...
              <div className="flex flex-col items-center gap-2 text-slate-500">
                ...
                <input
                  ...
                  className="mt-2 w-full max-w-md px-3 py-2 border border-slate-200 rounded-lg text-xs text-slate-700 focus:outline-none focus:ring-1 focus:ring-primary bg-white text-center"
                />
              </div>
            )}
          </div>
    ...
    <div className="w-full lg:w-72 flex-shrink-0">
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden sticky top-0 shadow-sm">
        ...
        <div className="p-4">
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            ...
            {form.suggested_amounts.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-3">
                {form.suggested_amounts.slice(0, 3).map(a => (
          ...
          <p className="text-[10px] text-slate-soft text-center mt-3">Approximate preview — final appearance may vary</p>
        </div>
      </div>
    </div>
  </div>
