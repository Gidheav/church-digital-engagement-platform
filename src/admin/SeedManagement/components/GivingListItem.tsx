...
const GivingListItem = memo<GivingListItemProps>(({
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className={`bg-white rounded-xl border border-slate-200 hover:border-slate-300 hover:shadow-sm transition-all select-none ${isDragging ? 'opacity-40 scale-[0.99]' : ''}`}
    >
      <div className="flex items-center gap-4 px-5 py-4">
        ...
        {/* Order number */}
        <span className="text-xs font-bold text-slate-300 w-5 text-center flex-shrink-0">{index + 1}</span>

        {/* Icon */}
        <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center flex-shrink-0">
          <Icon name={normalizeIconName(supportedIcons, item.icon)} size={20} className="text-slate-soft" />
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-0.5">
            ...
            {item.is_featured && (
              <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[9px] font-bold uppercase tracking-wider border border-amber-200">Featured</span>
            )}
          </div>
          <div className="flex items-center gap-3 text-xs text-slate-soft flex-wrap">
            ...
            <span className="w-1 h-1 rounded-full bg-slate-300" />
            <span className="font-semibold text-slate-700">{formatCurrency(item.total_donations)} raised</span>
            {item.goal_amount && (
              <>
                <span className="w-1 h-1 rounded-full bg-slate-300" />
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <button
            ...
            className={`p-2 rounded-lg transition-colors ${item.is_featured ? 'text-amber-500 bg-amber-50 hover:bg-amber-100' : 'text-slate-300 hover:text-amber-400 hover:bg-amber-50'}`}
