    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-base font-bold text-slate-900">Delete Giving Item</h2>
            <p className="text-xs text-slate-500">This action is permanent and cannot be undone.</p>
          </div>
        </div>
        <p className="text-sm text-slate-700 mb-4">
          You are about to permanently delete <strong className="text-slate-900">"{item.title}"</strong>. All associated data and donation records will be detached.
        </p>
        <p className="text-xs text-slate-500 mb-1.5">
          To confirm, type <strong className="text-slate-800">{item.title}</strong> below:
        </p>
        <input
          type="text"
          value={confirmInput}
          ...
          className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm mb-5 focus:outline-none focus:ring-1 focus:ring-primary"
          autoFocus
        />
